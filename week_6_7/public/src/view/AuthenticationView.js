...
export default class AuthenticationView extends View
{
	...
	passwordVerificationFormHandler;
	mobileVerificationFormHandler;
	registrationFormHandler;
	restorePasswordFormHandler;

	/* form state */
	passwordVerificationFormState = {input: {}, errors: {}};
	mobileVerificationFormState = {input: {}, errors: {}};
	...
	update()
	{
		...
		if(this.isLoginForm) {
			const passwordVerificationForm = this.createPasswordVerificationForm();
			const mobileVerificationForm = this.createMobileVerificationForm();
			content.append(passwordVerificationForm, mobileVerificationForm);
		}
		else {
	...
	createPasswordVerificationForm()
	{
		const form = this.createForm();
		const state = this.passwordVerificationFormState;
		...
		this.setFormHandler(form, input => {
			this.passwordVerificationFormHandler(
				input.email.value,
	...
	createMobileVerificationForm()
	{
		const form = this.createForm();
		const state = this.mobileVerificationFormState;
		form.append(
		...
		this.setFormInputListeners(form, state.input);
		this.setFormHandler(form, input => {
			this.mobileVerificationFormHandler(input.phone_number.value);
		});
